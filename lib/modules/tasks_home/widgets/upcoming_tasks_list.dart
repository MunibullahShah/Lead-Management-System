    ...
    return Obx(
      () => APIStateWidget(
          ...
          successWidget: RefreshIndicator(
            onRefresh: () async {
              await controller.getTasks(state: TaskState.upcoming);
            },
            child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return InkWell(
                    ...
                    onTap: () {
                      Get.to(
                        () => TaskDetailView(
                          taskState: controller.upcomingTasksState.value.model!
                                  .tasks![index].taskState ??
                              TaskState.overdue,
                        ),
                        arguments: controller
                            .upcomingTasksState.value.model!.tasks?[index],
                      );
                    },
                    child: TaskCard(
                      context: context,
                      heading: controller.upcomingTasksState.value.model!
                              .tasks![index].title ??
                          "",
                      subHeading: controller.upcomingTasksState.value.model!
                              .tasks![index].subTitle ??
                          "",
                      dateTime: controller.upcomingTasksState.value.model!
                              .tasks![index].dateTime ??
                          "",
                      taskState: controller.upcomingTasksState.value.model!
                              .tasks![index].taskState ??
                          TaskState.overdue,
                      hasOpened: controller.upcomingTasksState.value.model!
                ...
                itemCount:
                    controller.upcomingTasksState.value.model?.tasks?.length ??
                        0),
          ),
          ...
          generalApiState: controller.upcomingTasksState.value),
    );
  }
}
